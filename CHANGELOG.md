...
- **Wuji Hand 2**: Hands shipped from October 2026 can turn joint feed-forward compensation on or off. On hands shipped earlier, the switches are available, but control behavior doesn't change.
...
- **Wuji Hand 2 — BREAKING**: hands running firmware 2.7.0 or later accept SDK connections on port 50001 instead of port 7447. SDK device discovery picks the right port automatically. Applications that connect to port 7447 directly must switch to 50001, and firewall rules that only allow 7447 need updating.
