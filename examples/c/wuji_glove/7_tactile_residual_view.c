/*
 * Wuji SDK C - WujiGlove live tactile residual view.
 *
 * Standalone post-calibration observation tool: renders the `tactile_residual`
 * stream as a 24x31 terminal grid, the C counterpart of the Python example
 * `9.tactile_residual_view.py`. It is not an alternative
 * tactile-calibration workflow.
 *
 * `tactile_residual` is the continuous signal behind `tactile_binary`: a signed
 * per-taxel residual (positive = harder than the calibrated baseline, ~0 =
 * none, negative = lighter). Unlike `tactile_binary`, contact detection is left
 * to the consumer -- you apply your own threshold. Adjust it live with + / -;
 * the printed peak value guides where to set it.
 *
 ...
 *          cmake --build build
 * Run:   ./build/7_tactile_residual_view <glove_serial> [seconds] [threshold]
 */
...
#define MASKED_VALUE (-1.0f)  // invalid/masked sentinel
#define MASKED_EPS (1e-6f)
#define THRESHOLD_MIN_TENTHS 0
#define THRESHOLD_DEFAULT_TENTHS 5

// The residual magnitude ramp for taxels below the threshold (low -> high).
...
static atomic_int g_threshold_tenths = THRESHOLD_DEFAULT_TENTHS;
...
static int clamp_threshold_tenths(int tenths) {
    if (tenths < THRESHOLD_MIN_TENTHS) return THRESHOLD_MIN_TENTHS;
    return tenths;
}

static char cell_for(float value, float threshold) {
    if (fabsf(value - MASKED_VALUE) < MASKED_EPS) return ' ';
    if (threshold > 0.0f && value >= threshold) return '#';
    if (value <= 0.0f || threshold <= 0.0f) return '.';
    int idx = (int)(value / threshold * RAMP_LEN);
...
static void render_frame(const WujiTactileResidual *frame) {
    const size_t expected = TACTILE_ROWS * TACTILE_COLS;
    const size_t limit = frame->data_len < expected ? frame->data_len : expected;
    const float threshold = (float)atomic_load(&g_threshold_tenths) / 10.0f;
    size_t above = 0;
    float peak = -1e30f;
    size_t peak_idx = 0;
    ...
    for (size_t i = 0; i < limit; i++) {
        ...
        if (fabsf(v - MASKED_VALUE) < MASKED_EPS) continue;
        if (threshold > 0.0f && v >= threshold) above++;
        if (v > peak) {
            peak = v;
            peak_idx = i;
        }
    }
    if (peak <= -1e30f) peak = 0.0f;

    printf("\x1b[H\x1b[2J");
    printf("above=%-3zu threshold=%.1f peak=%+.3f row=%zu col=%zu seq=%u len=%zu  (+/- threshold, q quit)\n",
           above,
           (double)threshold,
           (double)peak,
...
static void on_residual(WujiFrameKind kind, const WujiTactileResidual *frame, void *userdata) {
    (void)userdata;
    static long last_render_ms = 0;

    if (kind == WUJI_FRAME_KIND_LAG) return;
    if (kind == WUJI_FRAME_KIND_END) {
        printf("\n[end] residual stream ended\n");
        atomic_store(&g_stop, 1);
        return;
    }
    if (kind == WUJI_FRAME_KIND_ERROR) {
        printf("\n[error] residual stream error\n");
        atomic_store(&g_stop, 1);
        return;
    }
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <glove_serial> [seconds] [threshold]\n", argv[0]);
        return 2;
    }
    ...
    int threshold = (argc >= 4)
        ...
        : THRESHOLD_DEFAULT_TENTHS;
    threshold = clamp_threshold_tenths(threshold);
    atomic_store(&g_threshold_tenths, threshold);
    if (seconds < 0.0) {
    ...
    printf("Subscribing to tactile_residual. Calibrate the glove first if the stream stays near zero.\n");
    ...
    if (wuji_glove_subscribe_tactile_residual(dev, on_residual, NULL, &sub) != WUJI_STATUS_OK) {
        print_error("wuji_glove_subscribe_tactile_residual");
    ...
    printf("\x1b[H\x1b[2Jwaiting for tactile_residual frames... (+/- threshold, q quit)\n");
    ...
    while (!atomic_load(&g_stop)) {
        ...
        if (key == 'q' || key == 'Q') {
            ...
            atomic_store(&g_threshold_tenths, clamp_threshold_tenths(atomic_load(&g_threshold_tenths) + 1));
        } else if (key == '-' || key == '_') {
            atomic_store(&g_threshold_tenths, clamp_threshold_tenths(atomic_load(&g_threshold_tenths) - 1));
        }
